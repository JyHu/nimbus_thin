...
+ (BOOL)compatibleOpenURL:(NSURL *)url
{
    if ([[UIApplication sharedApplication] canOpenURL:url])
    {
#if __IPHONE_OS_VERSION_MAX_ALLOWED > NSFoundationVersionNumber_iOS_9_x_Max
#warning - 这样直接返回yes是不合理的，因为handler是一个block，成功打开的结果在block里
...
+ (BOOL)safariWithURL:(NSURL *)url {
    return [self compatibleOpenURL:url];
}
...
+ (BOOL)googleChromeWithURL:(NSURL *)url {
    ...
    if (chromeScheme) {
        ...
        BOOL didOpen = [self compatibleOpenURL:chromeUrl];
        if (!didOpen) {
...
+ (BOOL)googleMaps {
    BOOL didOpen = [self compatibleOpenURL:[NSURL URLWithString:sGoogleMapsScheme]];
...
+ (BOOL)phoneWithNumber:(NSString *)phoneNumber {
    ...
    return [self compatibleOpenURL:[NSURL URLWithString:urlPath]];
}
...
+ (BOOL)iBooks {
    BOOL didOpen = [self compatibleOpenURL:[NSURL URLWithString:sIBooksScheme]];
...
+ (BOOL)facebook {
    BOOL didOpen = [self compatibleOpenURL:[NSURL URLWithString:sFacebookScheme]];
...
+ (BOOL)twitter {
    BOOL didOpen = [self compatibleOpenURL:[NSURL URLWithString:sTwitterScheme]];
...
+ (BOOL)applicationWithScheme:(NSString *)applicationScheme
    ...
    didOpen = [self compatibleOpenURL:[NSURL URLWithString:urlPath]];
...
+ (BOOL)instagram {
    BOOL didOpen = [self compatibleOpenURL:[NSURL URLWithString:sInstagramScheme]];
